using Com.Core;
using Com.Input.CameraController.Foundation;
using Com.Input.EventSystem;
using UnityEngine;
using static UnityEngine.Input;
using static UnityEngine.Mathf;

namespace Com.Input.CameraController
{
    /// <summary>
    /// 附加在相机上的控制器实现
    /// </summary>
    ...
    [AddComponentMenu("Com/Input/EditorLikeCameraController", 11)]
    public class EditorLikeCameraController : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            var inRange = pos.x >= 0 && pos.x <= Screen.width && pos.y >= 0 && pos.y <= Screen.height;
        ...
        static Vector2 ScreenCoordDelta => MouseTouchInputModule.Instance.MouseTouchData.ScreenCoordDelta;
        ...
        void SetDragDepth()
        {
            ...
            bool _hitAny = Physics.Raycast(cameraForward, out RaycastHit hitInfo, farClipPlane);

            if (enableDragStart)
            {
                var planecast = new Plane(Vector3.up, 0).Raycast(cameraForward, out var planeEnter) ? Abs(planeEnter) : float.PositiveInfinity;
                if (_hitAny)
                {
        ...
        {
            var power = MovePower;
            var rotateMul = arrowKeyRotatePower * Time.deltaTime * (reverseRotate ? -1 : 1);
        ...
        void MoveForward()
        {
            ...
            if (scrollDelta != 0)
            {
                ...
                {
                    float nearClipPlane = m_camera.nearClipPlane, farClipPlane = m_camera.farClipPlane;
                    float _forwardDepth = Physics.Raycast(m_camera.ScreenPointToRay(mousePosition), out RaycastHit hitInfo, farClipPlane)
