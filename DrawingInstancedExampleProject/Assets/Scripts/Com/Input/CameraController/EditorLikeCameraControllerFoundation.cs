...
using Com.Core;
using UnityEngine;
using static Com.Windows.CursorUtil;
using static Com.Windows.User32;

namespace Com.Input.CameraController.Foundation
{
    /// <summary>
    /// 相机控制器保存的状态
    /// </summary>
    public struct EditorLikeCameraControllerContext
    {
        ...
        public Vector3 ViewportToWorldPoint(in Vector2 viewportPosition)
        {
        ...
        public Vector3 ScreenToWorldPoint(in Vector2 mousePosition)
        {
    ...
    /// </summary>
    public struct EditorLikeCameraActionProcess
    {
        ...
        public bool Valid => actionFlag != ActionType.None;
        ...
        public bool Execute()
        {
            ...
            Vector2 coord = point - roundCursorDelta;

            Vector2 _coordDelta = coord - screenCoord;
            // 矫正突变
        ...
        {
            Vector2 pointCoord = Vector2.Scale(coordDelta, __scaleReverseY) + mousePosition;
            Vector3 worldDelta = mouseWorldPivot - ctx.ScreenToWorldPoint(pointCoord);
            ctx.position += worldDelta;
        }

        private void RotateView(in Vector2 coordDelta)
        {
            Vector2 delta = coordDelta;
        ...
        private void RotateAroundView(in Vector2 coordDelta)
        {
            ...
            float _x = initEulerAngles.x + delta.y;
            float _y = initEulerAngles.y + delta.x;
            Vector3 euler = new Vector3(_x, _y);
